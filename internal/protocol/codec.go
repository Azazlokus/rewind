...
func DecodeServer(data []byte, out *ServerMessage) error {
	...
	case MsgMatchState:
		...
		for range count {
			// Фикс. часть строки табло — 10 байт: [2B id][2B kills][2B deaths][1B team]
			// [2B hillScore][1B nameLen].
			if len(body) < 10 {
			...
			s := MatchScore{
				...
				Team:      body[6],
				HillScore: binary.LittleEndian.Uint16(body[7:9]),
			}
			n := int(body[9])
			if n > MaxNameLen {
			...
			if len(body) < 10+n {
				return fmt.Errorf("%w: score name %d bytes, got %d", ErrShortMessage, n, len(body)-10)
			}
			name := body[10 : 10+n]
			if !utf8.Valid(name) {
...
func AppendMatchState(dst []byte, m MatchState) ([]byte, error) {
	...
	for i := range m.Scores {
		...
		dst = binary.LittleEndian.AppendUint16(dst, s.HillScore)
