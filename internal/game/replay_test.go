...
func TestDecodeReplayRejectsGarbage(t *testing.T) {
	...
	// Лог с заявленным событием, но без тела. eventCount в v6 (итер. 31) — по смещению
	// 25 (после байтов teamMode@21, hillMode@22, domMode@23 и ctfMode@24).
	claim := (&ReplayLog{Seed: 1, TickRate: 30}).Encode()
	claim[25] = 1 // eventCount = 1, а тела нет
