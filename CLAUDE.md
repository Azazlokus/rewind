...
## Статус: дорожная карта пройдена; persister (итер. 14B)

Итерации 1–11 (исходное ТЗ) + 12 (WebRTC до продакшена) + 13 (фундамент бэкенда) + 14 (жизненный цикл
матча) + 14B (persister) сделаны. Дальнейшие работы — по новым запросам; тот же воркфлоу на feature-ветке:
ветка → исследовать → план → код → /audit → `make check` → коммит → push → PR → merge зелёным, отчёт и
