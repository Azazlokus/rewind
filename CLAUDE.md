...
5. Комментарии и идентификаторы в коде — английские. **Commit-сообщения —
   на русском, по Conventional Commits** (`type(scope): описание` в императиве,
   scope = пакет/область). Документация (`docs/`, README) — на русском; README
   дублируется на английский в `README.en.md`, обе версии держать синхронными.
...
См. план по итерациям в исходном ТЗ. Итерация 1 (скелет: двигающиеся точки) — в
работе. Не начинать следующую итерацию, пока не выполнены критерии приёмки
текущей и не зелёный `make check`.
