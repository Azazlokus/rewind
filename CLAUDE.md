...
См. план по итерациям в исходном ТЗ. Итерация 1 (скелет) и итерация 2
(интерполяция) — сделаны. Сервер шлёт снапшоты 20 Гц (делитель частоты по
Брезенхэму), клиент рендерит на 100 мс в прошлом с интерполяцией. Следующая —
итерация 3 (бинарный протокол). Не начинать следующую итерацию, пока не выполнены
критерии приёмки текущей и не зелёный `make check`.
