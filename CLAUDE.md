...
## Статус: дорожная карта пройдена; киллстрики + неуязвимость (итер. 20)
...
аптечки, ускорение, веер) + 20 (киллстрики + окно неуязвимости) сделаны. Дальнейшие работы — по новым запросам;
тот же воркфлоу на feature-ветке: ветка → исследовать → план → код → /audit → `make check` → коммит → push →
PR → merge зелёным, отчёт и BENCHMARKS/docs по правилу 7.
