...
окна — анти-чит); история и `rewind` снаряда входят в `Checksum`. Приёмка зелёная
(`make check` + `make integration`). Следующая — **итерация 6** (spatial hash
grid, interest management, дельты, нагрузка 200 ботов); не начинать без запроса.
