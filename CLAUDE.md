...
## Статус: дорожная карта пройдена; рейт-лимит на auth (итер. 21)
...
аптечки, ускорение, веер) + 20 (киллстрики + окно неуязвимости) + 21 (рейт-лимит на auth) сделаны. Дальнейшие
работы — по новым запросам; тот же воркфлоу на feature-ветке: ветка → исследовать → план → код → /audit →
`make check` → коммит → push → PR → merge зелёным, отчёт и BENCHMARKS/docs по правилу 7.
