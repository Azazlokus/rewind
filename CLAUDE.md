...
Сделана **итерация 41** — codegen клиентских констант (`cmd/genclient`, `internal/protocol/clientgen.go`,
`internal/game/clientgen.go`, `web/game.js`, `Makefile`, docs). Убрано ручное зеркалирование констант в клиенте —
источник дрейфа (напр. сломанный разбор `MsgMatchState` в итер. 23). Теперь источник истины — Go, а генератор
переносит значения в клиент. **Формат провода НЕ менялся** (`codec.go`/`protocol.go` не тронуты) — это чистый
рефактор зеркала. **Механизм.** Поверхности `protocol.ClientWire()` (коды сообщений, `Btn*`, `weaponSelectShift/
Mask`, `ActDash`, `Field*`/`FieldAll`, `matchFlag*`, `CoordScale`) и `game.ClientSim()` (движение/рывок, `InputRate`,
геометрия `walls`/`pickupSpots`/`domPoints`/`flagBases`/hill, enum'ы `matchPhase`/`pickupKind`/`weaponKind`) читают
РЕАЛЬНЫЕ значения в своём пакете (в т.ч. на `iota` и неэкспортированные) — read-only, срезы копируются (как
`Obstacles()` итер. 28). `cmd/genclient` рендерит JS-блок и вписывает его в `web/game.js` между `// GENERATED-BEGIN`/
`// GENERATED-END` (путь через `runtime.Caller` — cwd-независимо, `spliceBlock` идемпотентен). **Дрейф ловит
golden-тест** `cmd/genclient` в `make check` (проверено негативным прогоном: падает при расхождении, проходит после
`make gen`). Регенерация — `make gen` / `//go:generate go run .`. **Покрыто генерацией:** `PROTO` (коды 0x01..0x1a,
enum'ы matchPhase/pickup/weapon, биты Btn/Field, WeaponSelectShift/Mask, ActDash), `MATCH_*_MODE`, `COORD_SCALE`,
`INPUT_RATE`, `INV_SQRT2`, `SIM` (движение + hill/dom/ctf-геометрия), `WALLS`, `PICKUP_SPOTS`. Значения совпали со
старым ручным зеркалом БАЙТ-В-БАЙТ (единственное изменение — `INV_SQRT2` `0.70710678→0.70710677`, точный float32
серверного `invSqrt2`; клиент, float64, вне Checksum). Клиент отрефакторен ссылаться на сгенерированное:
`weaponKeyMap`/`WEAPON_NAMES`→`PROTO.Weapon*`, маска→`PROTO.WeaponSelectMask`, `PREDICT.dt`→`1/INPUT_RATE`. **Вне
маркеров** остаётся ручной рендер/UX (цвета/глифы/`TEAM_COLORS`, длительности баннеров) и `INTERP.tickRate`
(рантайм-конфиг `ARENA_TICK_RATE`, а не compile-time const). **Три стража чисты:** concurrency (новых горутин/каналов
нет, аксессоры read-only, срезы копируются, модель «одна комната = одна горутина» не задета, `-race`+vet зелёные),
determinism (значения симуляционных констант не менялись — сверены у источника, `Step`/состояние/`Checksum` не
тронуты, новый код вне горячего пути, серверный `invSqrt2` неизменен), protocol (вердикт «СИНХРОНИЗИРОВАНО»,
байт-в-байт включая раскладку `MsgMatchState` — критичное ребро итер. 23 чисто; `protocol` не тянет игровое/сетевое;
zero-alloc/fuzz зелёные; генератор идемпотентен). **Находки protocol-guardian (не дефекты)** закрыты в итерации тем
же конвейером: коды оружия (finding 2, wire-значимы) и `weaponSelectMask` (finding 3) добавлены в генератор; **отложен
finding 1** — codegen байтовых РАСКЛАДОК сообщений (другой механизм; смещения полей в декодере остаются ручными,
задокументировано в `docs/protocol.md` как ограничение). Приёмка зелёная (`make check` + `node --check` + негативный
drift-тест). `BENCHMARKS.md` не трогался — горячего пути нет. Docs: README RU/EN (`make gen`), `docs/protocol.md`
(секция кодогенерации + оговорка о раскладках), заметка в «Общем стиле работы».

## Статус: дорожная карта режимов пройдена; идёт инфра/DevEx-программа (32 — стек наблюдаемости, 41 — codegen констант)
...
Capture the Flag). Далее — программа по новым запросам (инфра/прод, бэкенд/аккаунты, качество/DevEx): сделаны
**32** (локальный стек наблюдаемости — docker-compose + Prometheus + Grafana) и **41** (codegen клиентских
констант из Go — `web/game.js` больше не зеркалит провод/геометрию руками). Тот же воркфлоу на
...
- Перед правкой формата протокола или движения — помнить, что клиент (`web/game.js`)
  зеркалит константы. С итерации 41 блок между `// GENERATED-BEGIN`/`// GENERATED-END`
  в `web/game.js` **генерируется** из `internal/protocol`+`internal/game` командой
  `make gen` (`cmd/genclient`): менять значение в Go → `make gen`, а не править клиент
  руками. Дрейф ловит golden-тест `cmd/genclient` в `make check`. Вне маркеров —
  ручной клиентский код (рендер/UX), его правят как обычно.
