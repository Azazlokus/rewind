...
> Статус: **итерация 19 — оружие/пикапы** (аптечки, ускорение стрельбы и веер на
> фиксированных точках; спавн детерминирован `w.rng`, буфы и состояние точек в `Checksum`;
> провод — отдельный reliable `MsgPickupState`, снапшот не тронут; клиент рисует пикапы).
> До этого: звук боя (выстрел/попадание/смерть/фраг/респаун через Web Audio на уже
