...
> Статус: **итерация 16 — профиль игрока** (модалка со статистикой и историей матчей;
> клик по строке лидерборда или своей кнопке — чистый фронтенд поверх готового REST).
> До этого: клиент/UX (экран логина/регистрации на REST-бэкенде с токен-сессией,
> лидерборд, миникарта — чистый canvas/JS без сборщиков, итер. 15), persister (комната
> шлёт смерти и итоги матчей в канал, `internal/persist` пишет
