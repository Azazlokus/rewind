...
> Статус: **итерация 25 — античит-метрики** (Prometheus `arena_anticheat_events_total{kind}`:
> считает срабатывания серверного клампа перемотки — `ViewTick` из будущего или дальше окна;
> наблюдение поверх существующего анти-чита, счётчики во `World` вне `Checksum`, слив комнатой
> в `Recorder` после тика). До этого: мобильное управление (твин-стик поверх canvas: левый стик —
