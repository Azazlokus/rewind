...
> Статус: **итерация 1 — скелет (двигающиеся точки)**. План — в `CLAUDE.md`.
...
3. Подключитесь в обеих. Двигайтесь WASD в одной — во второй этот игрок (красный)
   движется в реальном времени, и наоборот.

В итерации 1 мир рендерится прямо из последнего снапшота сервера, поэтому под
искусственной задержкой чужие игроки дёргаются. Сглаживание (интерполяция) и
мгновенный отклик своего персонажа (предсказание) появятся в итерациях 2 и 4.
...
| `ARENA_SNAPSHOT_RATE`    | `30`               | частота снапшотов, Гц (станет 20 в итер. 2) |
