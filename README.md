...
> Статус: **итерация 21 — рейт-лимит на auth** (пер-IP токен-бакет на `/api/register`/`login`/
> `guest` — брут-форс/спам ограничены, 429 + `Retry-After`; конкурентно-безопасно, без фоновых
> горутин; включён по умолчанию, настраивается через env). До этого: киллстрики + окно
