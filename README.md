...
> Статус: **итерация 29 — King of the Hill** (режим захвата зоны: в центре арены круглый
> холм; пока его контролирует ровно одна сторона — команда в командном режиме, иначе игрок —
> её бойцы в зоне копят `HillScore`, при оспаривании очки не растут; победитель матча — по
> очкам холма; `HillScore` в `Checksum`, флаг `hillMode` — параметр мира в логе реплея v4;
> холм едет клиенту через `MsgMatchState`, снапшот не тронут; клиент рисует зону и красит
> контролёра локально; `ARENA_HILL_MODE`). До этого:
...
Из режимов на будущее остаются **Capture the Flag** и **доминация** (несколько контрольных
точек) — по одному на PR.
