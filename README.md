...
> Статус: **итерация 14B — persister** (комната шлёт смерти и итоги матчей в канал,
> `internal/persist` пишет статистику/историю в БД вне горутины комнаты; джойн несёт
> токен-сессию — сессия привязывается к аккаунту). До этого: жизненный цикл матча (FFA
> deathmatch с таймером: раунд на время, счёт убийств/смертей, детерминированный
> победитель, антракт и авторестарт; табло/таймер/баннер победителя — итер. 14).
