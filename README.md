...
> Статус: **итерация 18 — звук** (выстрел/попадание/смерть/фраг/респаун через Web Audio
> на уже приходящих событиях боя — чистый фронтенд, синтез без ассетов, тумблер в HUD).
> До этого: серверные боты (наполнитель держит в занятой комнате `ARENA_BOT_FILL` игроков,
...
swarm) и гоняет автопилот из `internal/bot`; мир комнаты он **не трогает** — только
