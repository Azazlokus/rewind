...
> Статус: **итерация 30 — доминация** (несколько контрольных точек: прямое обобщение King of
> the Hill с одной зоны на N; на арене несколько круглых зон, для каждой действует та же логика
> контроля — ровно одна сторона внутри копит очки — и очки суммируются в `DomScore` по всем
> удерживаемым зонам; победитель матча — по сумме очков зон; `DomScore` в `Checksum`, флаг
> `domMode` — параметр мира в логе реплея v5; зоны едут клиенту через `MsgMatchState` (слот
> `objScore`, общий с холмом — режимы взаимоисключающи), снапшот не тронут; клиент рисует все
> зоны и красит контролёра каждой локально; совместимо с командным режимом; `ARENA_DOM_MODE`).
> До этого: King of the Hill (режим захвата одной зоны в центре арены; бойцы контролирующей
> стороны копят `HillScore`, победитель — по очкам холма; `HillScore` в `Checksum`, флаг
> `hillMode` — параметр мира в логе реплея v4; холм едет клиенту через `MsgMatchState`, снапшот
> не тронут; `ARENA_HILL_MODE`, итер. 29). До этого:
...
Из режимов на будущее остаётся **Capture the Flag** — по одному на PR.
