...
- `internal/account` — идентичность: гость + аккаунты (argon2id). Токены (итер. 36):
  ...
  **Email + сброс пароля (итер. 37):** при регистрации можно указать email — на него
  уходит одноразовый токен верификации; «забыл пароль» шлёт токен сброса, а сброс меняет
