...
> Статус: **итерация 27 — рывок** (dash по Space: короткое ускорение в сторону движения с
> кулдауном; input-driven и предсказывается клиентом; таймеры в `MoveState`/`Checksum`, бит
> `ActDash` в отдельном опциональном байте `Input.Actions`; сервер гейтит кулдаун — анти-чит).
> До этого: система оружия (4 типа: пистолет/дробовик/снайперка/ракета со сплэшем; выбор
...
> снапшот не тронут, итер. 26). Ещё раньше: античит-метрики (Prometheus
