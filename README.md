...
> Статус: **итерация 22 — спектатор/наблюдатель** (присоединиться к комнате без спавна: `Join`
...
> До этого: рейт-лимит на auth (пер-IP токен-бакет на `/api/register`/`login`/`guest` — брут-форс/
