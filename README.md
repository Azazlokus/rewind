...
> Статус: **итерация 17 — серверные боты** (наполнитель держит в занятой комнате
> `ARENA_BOT_FILL` игроков, подсаживая ИИ-ботов и уступая место людям — боты обычные
> клиенты через Pipe, мир не трогают). До этого: профиль игрока (модалка со статистикой
> и историей матчей — итер. 16), клиент/UX (экран логина/регистрации на REST-бэкенде с
> токен-сессией, лидерборд, миникарта — чистый canvas/JS без сборщиков, итер. 15),
