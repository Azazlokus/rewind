...
## Итерация 5 — бой (боевое ядро)

Добавлены снаряды, урон, смерть/респаун и reliable-события. Горячий путь остался
zero-alloc: id снарядов проверяются сканом слайса (без map), а очереди снарядов и
событий переиспользуются.

### Стоимость тика

| Benchmark              | ns/op  | B/op | allocs/op |
|------------------------|-------:|-----:|----------:|
| Tick / 50 (движение)   |   3711 |    0 |         0 |
| Tick / 200 (движение)  |  14985 |    0 |         0 |
| CombatTick / 50 (бой)  | 140100 |    0 |         0 |
| AppendEntities / 200   |   4511 |    0 |         0 |

`BenchmarkCombatTick` — 50 игроков стреляют каждый тик, ~128 снарядов в воздухе,
свит-коллизия каждого против каждого живого игрока: ~140 мкс/тик (0.4% бюджета
33 мс), **0 allocs/op**. Чистое движение (без боя) — как в итерации 4. Профиль
коллизии — O(снаряды × игроки), кандидат на широкофазную оптимизацию в итерации 6
(та же пространственная сетка, что нужна interest management).
