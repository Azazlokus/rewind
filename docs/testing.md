...
Комната умеет писать лог: seed + все вводы с номерами тиков (бинарный формат в
`testdata/replays/`). `cmd/replay` проигрывает лог headless и сверяет хэш
финального состояния. Каждый пойманный desync-баг превращается в replay-файл и
становится регрессионным тестом. На итерации 1 — задел (`cmd/replay` появится
позже).
...
- `BenchmarkTick` (комната на 50/200 сущностей), `BenchmarkEncodeSnapshot`,
  ...
  горячем пути — 0 allocs/op (симуляция уже zero-alloc; кодек — после итерации 3).
