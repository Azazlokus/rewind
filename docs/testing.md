...
- **Лог v2 (итер. 23):** заголовок несёт байт `teamMode` (командный режим —
  фиксированный параметр мира, в `Checksum` не входит, поэтому обязан ехать в логе).
  `Replay` восстанавливает его через `SetTeamMode` до первого join, иначе
  дружественный огонь реконструировался бы иначе и хэш разошёлся бы.
- **Лог v3 (итер. 29):** input-событие несёт байт `Actions` (действия/абилки — рывок
  `ActDash` из итер. 27 влияет на движение и входит в `Checksum`, поэтому обязан ехать
  в логе). До v3 реплей с рывками рассинхронивался — латентный баг итер. 27.
- Декодер принимает v1 (без байтов — FFA, без абилок), v2 (teamMode) и v3 (teamMode +
  Actions) — обратная совместимость старых логов.
...
итоговый мир), `TestReplayDeterministicAcrossRuns`, `TestReplayTeamModeRoundTrip`
(лог переносит `teamMode`), `TestReplayDashRoundTrip` (лог v3 переносит `Actions` —
рывок; негативный контроль: без `Actions` хэш расходится).
